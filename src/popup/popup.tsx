      ...
      <ul class="pr-list">
        <For each={allPRs()}>
          {(pr) => {
            ...
            const isUnseen = () =>
              !isSeen() || new Date(pr.updated_at) > new Date(isSeen()!);
            ...
            return (
              ...
              >
                <div class="pr-title">
                  <span
                    class={`status-dot ${isUnseen() ? "unseen" : "seen"}`}
                  />
                  <span class="pr-title-text">{pr.title}</span>
                  <span
                    class={`pr-type-label ${pr.type === "APPROVED" ? "approved" : ""}`}
                  >
                    {pr.type}
                  </span>
                </div>
...
function PopupContent() {
  ...
  const [seenPRs, { mutate: mutateSeenPRs, refetch: refetchSeenPRs }] =
    createResource(fetchSeenPRs);
