...
const prQuery = queryOptions({
  ...
  queryFn: async () => {
    ...
    const { previousPRs, seenPRs } = await chrome.storage.local.get([
      "previousPRs",
      "seenPRs",
    ]);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "prsUpdated") {
    queryClient.invalidateQueries(prQuery);
  }
});
...
function PRList({ activeTab }: { activeTab: string }) {
  const { data } = useSuspenseQuery(prQuery);

  const currentPRs = data.previousPRs[activeTab as keyof PRData] || [];
  const seenPRs = data.seenPRs;
